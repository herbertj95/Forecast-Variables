...
def forecast(data, var, season, start_forecast):
    ...
    data_final[var] = np.where(data_final.Hour < 5, 0, data_final[var])
    data_final[var] = np.where(data_final.Hour > 19, 0, data_final[var])
    ...
    plt.xlabel("Real generation (kWh)", alpha= 0.75, weight= "bold")
    plt.ylabel("Predicted generation (kWh)", alpha= 0.75, weight= "bold")
    plt.xticks(alpha= 0.75,weight= "bold",fontsize= 11)
    plt.yticks(alpha= 0.75,weight= "bold", fontsize= 11)
