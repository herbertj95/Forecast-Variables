...
def forecast(data, variables, season, start_forecast):
    ...
    for i, value in enumerate(data_final[variables[1]]):
        # Cheking if the 'Plugged' is 1, if it is, count the number of ones and distribute the 'Energy req' value among them
        if value == 1:
            count += 1
            if data_final[variables[2]][i] != 0:
                energy = data_final[variables[2]][i]
                replacement = energy / count
                data_final.loc[i-count+1:i+1, variables[2]] = replacement
                count = 0
                energy = None
            
    'Looking for the errors in the distribution of the energy'
    error = data_final[(data_final[variables[1]] == 0) & (data_final[variables[2]] != 0) | (data_final[variables[1]] == 1) & (data_final[variables[2]] == 0)]
    ...
    # mean_per_hour = data_final.groupby('Hour')[variables[2]].agg(["mean"])
    # fig, ax = plt.subplots()
    ...
    # plt.title(season+"Average energy consumption per hour", alpha= 0.75, weight= "bold", loc= "left", pad= 10)
    # plt.show()
...
###############################################################################################################################
    ...
    all_features.remove(variables[2])
    all_features= np.array(all_features) 
    
    'Moving the Plug column to the beginning of the dataframe'
    ...
    plt.title(season+ " correlation real vs predicted for "+ variables[1], alpha= 0.75, weight= "bold", pad= 10, loc= "left")
    plt.show()
    ...
    'Plot train-test'
    fig,ax = plt.subplots()
    coloring = data_final[variables[2]].max()
    plt.plot(data_train.index, data_train[variables[2]], color= "darkcyan", alpha= 0.75)
    plt.fill_between(data_train.index, coloring, facecolor= "darkcyan", alpha= 0.2)
    plt.plot(data_test.index, data_test[variables[2]], color = "dodgerblue", alpha= 0.60)
    plt.fill_between(data_test.index, coloring, facecolor= "dodgerblue", alpha= 0.2)
    plt.xlabel("Date", alpha= 0.75, weight= "bold")
    plt.ylabel("Energy (kWh)", alpha= 0.75, weight= "bold")
    plt.xticks(alpha= 0.75,weight= "bold", fontsize= 11)
    plt.yticks(alpha= 0.75,weight= "bold", fontsize= 11)
    plt.legend(['Train','Test'], frameon= False, loc= 'upper center', ncol= 2)
    plt.title(season+ " Train - Test split for " + variables[2], alpha= 0.75, weight= "bold", pad= 10, loc= "left")
    plt.show()
    ...
    first_column = data_final2.pop(variables[2])
    data_final2.insert(0, variables[2], first_column)
        
    X = data_final2.values
    Y = X[:, 0] 
    ...
    if variables[1] in USE_COLUMNS:
        xtest[variables[1]] = df_cla.Prediction
    ...
    plt.xticks(alpha= 0.75, weight= "bold", fontsize= 11)
    plt.yticks(alpha= 0.75, weight= "bold", fontsize= 11)
    plt.title(season+" correlation real vs predictions for "+ variables[2], alpha= 0.75, weight= "bold", pad= 10, loc= "left")
    plt.show()
    ...
    'Features used'
    USE_COLUMNS = all_features
    print('The features used in the Logistic Regression model are:', USE_COLUMNS)
    ...
    #Using the forecasted values of Plug and Energy
    if variables[1] in USE_COLUMNS:
        xtest[variables[1]] = df_cla.Prediction
        
    if variables[2] in USE_COLUMNS:
        xtest[variables[2]] = df_reg.Prediction
    ...
    predictions_plug_energy = df_reg[[variables[1]]]
    predictions_plug_energy[variables[2]] = df_reg.Prediction
    predictions_plug_energy[variables[0]] = df_cla2.Prediction
    predictions_plug_energy = predictions_plug_energy.reindex(columns= [variables[0], variables[1], variables[2]])
    
    'Post-processing'
    predictions_plug_energy[variables[0]] = np.where(predictions_plug_energy[variables[1]] == 0, 0, predictions_plug_energy[variables[0]])
    
    count = 0
    energy_cum = 0
    
    for i, value in enumerate(predictions_plug_energy[variables[1]]):
        # Cheking if the 'Plugged' is 1, if it is, count the number of ones and sum the energy
        if value == 1:
            first_zero = False
            user = predictions_plug_energy[variables[0]][i]
            if user != predictions_plug_energy[variables[0]][i-1] and predictions_plug_energy[variables[0]][i-1] != 0:
                predictions_plug_energy.loc[i-count:i-1, variables[2]] = 0
                predictions_plug_energy[variables[2]][i-1] = energy_cum
                count = 1
                energy_cum = predictions_plug_energy[variables[2]][i]
            else:
                count += 1
                energy_cum += predictions_plug_energy[variables[2]][i]
        ...
        if i == len(predictions_plug_energy)-1 and value == 1:
            first_zero = True
            predictions_plug_energy.loc[i-count:i, variables[2]] = 0
            predictions_plug_energy[variables[2]][i] = energy_cum
            count = 0
            energy_cum = 0
